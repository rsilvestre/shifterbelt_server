...
var Bootstrap = (function () {
    ...
    _createClass(Bootstrap, [{
        ...
        value: function run() {
            ...
            var p = dbloader(config.adapters.getAdapter("database")).then(function () {
                return dbloader(config.adapters.getAdapter("memory"));
            }).then(function () {
                return dbloader(config.adapters.getAdapter("queue"));
            }).then(function (rabbitAdapter) {
                var adapters = rabbitAdapter.getAdapters();
                var app = new _this._application(adapters);
            });
        }
    }]);
